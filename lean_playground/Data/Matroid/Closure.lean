...
In theorem names, the words `spanning` and `flat` are used as suffixes,
...
theorem flat_iff_isClosed : M.Flat F ↔ ∃ h : F ⊆ M.E, M.subtypeClosure.IsClosed ⟨F, h⟩ := by
  simpa [subtypeClosure] using Flat.subset_ground

theorem isClosed_iff_flat {F : Iic M.E} : M.subtypeClosure.IsClosed F ↔ M.Flat F := by
  simp [subtypeClosure]
...
theorem closure_def (M : Matroid α) (X : Set α) : M.closure X = ⋂₀ {F | M.Flat F ∧ X ∩ M.E ⊆ F} := rfl

theorem closure_def' (M : Matroid α) (X : Set α) (hX : X ⊆ M.E := by aesop_mat) :
...
theorem closure_subset_ground (M : Matroid α) (X : Set α) : M.closure X ⊆ M.E :=
  sInter_subset_of_mem ⟨M.ground_flat, inter_subset_right⟩

@[simp] theorem ground_subset_closure_iff : M.E ⊆ M.closure X ↔ M.closure X = M.E := by
  simp [M.closure_subset_ground X, subset_antisymm_iff]

@[simp] theorem closure_inter_ground (M : Matroid α) (X : Set α) :
    M.closure (X ∩ M.E) = M.closure X := by
  simp_rw [closure_def, inter_assoc, inter_self]

theorem inter_ground_subset_closure (M : Matroid α) (X : Set α) : X ∩ M.E ⊆ M.closure X := by
  simp_rw [closure_def, subset_sInter_iff]; simp

theorem mem_closure_iff_forall_mem_flat (X : Set α) (hX : X ⊆ M.E := by aesop_mat) :
    e ∈ M.closure X ↔ ∀ F, M.Flat F → X ⊆ F → e ∈ F := by
  simp_rw [M.closure_def' X, mem_sInter, mem_setOf, and_imp]

theorem subset_closure_iff_forall_subset_flat (X : Set α) (hX : X ⊆ M.E := by aesop_mat) :
    Y ⊆ M.closure X ↔ ∀ F, M.Flat F → X ⊆ F → Y ⊆ F := by
  simp_rw [M.closure_def' X, subset_sInter_iff, mem_setOf, and_imp]

theorem subset_closure (M : Matroid α) (X : Set α) (hX : X ⊆ M.E := by aesop_mat) :
    X ⊆ M.closure X := by
  simp [M.closure_def' X, subset_sInter_iff]

theorem Flat.closure (hF : M.Flat F) : M.closure F = F :=
  (sInter_subset_of_mem (by simpa)).antisymm (M.subset_closure F)

@[simp] theorem closure_ground (M : Matroid α) : M.closure M.E = M.E :=
  (M.closure_subset_ground M.E).antisymm (M.subset_closure M.E)

@[simp] theorem closure_univ (M : Matroid α) : M.closure univ = M.E := by
  rw [← closure_inter_ground, univ_inter, closure_ground]

@[gcongr]
theorem closure_subset_closure (M : Matroid α) (h : X ⊆ Y) : M.closure X ⊆ M.closure Y :=
  subset_sInter (fun _ h' ↦ sInter_subset_of_mem
    ⟨h'.1, subset_trans (inter_subset_inter_left _ h) h'.2⟩)

theorem closure_mono (M : Matroid α) : Monotone M.closure :=
  fun _ _ ↦ M.closure_subset_closure

@[simp] theorem closure_closure (M : Matroid α) (X : Set α) : M.closure (M.closure X) = M.closure X :=
...
theorem closure_subset_closure_of_subset_closure (hXY : X ⊆ M.closure Y) :
    M.closure X ⊆ M.closure Y :=
  (M.closure_subset_closure hXY).trans_eq (M.closure_closure Y)

theorem closure_subset_closure_iff_subset_closure (hX : X ⊆ M.E := by aesop_mat) :
    M.closure X ⊆ M.closure Y ↔ X ⊆ M.closure Y :=
  ⟨(M.subset_closure X).trans, closure_subset_closure_of_subset_closure⟩

theorem subset_closure_of_subset (M : Matroid α) (hXY : X ⊆ Y) (hY : Y ⊆ M.E := by aesop_mat) :
    X ⊆ M.closure Y :=
  hXY.trans (M.subset_closure Y)

theorem subset_closure_of_subset' (M : Matroid α) (hXY : X ⊆ Y) (hX : X ⊆ M.E := by aesop_mat) :
    X ⊆ M.closure Y := by
  rw [← closure_inter_ground]; exact M.subset_closure_of_subset (subset_inter hXY hX)

theorem exists_of_closure_ssubset (hXY : M.closure X ⊂ M.closure Y) : ∃ e ∈ Y, e ∉ M.closure X := by
  by_contra! hcon
  exact hXY.not_subset (M.closure_subset_closure_of_subset_closure hcon)

theorem mem_closure_of_mem (M : Matroid α) (h : e ∈ X) (hX : X ⊆ M.E := by aesop_mat) :
    e ∈ M.closure X :=
  (M.subset_closure X) h

theorem mem_closure_of_mem' (M : Matroid α) (heX : e ∈ X) (h : e ∈ M.E := by aesop_mat) :
    e ∈ M.closure X := by
  rw [← closure_inter_ground]
  exact M.mem_closure_of_mem ⟨heX, h⟩

theorem not_mem_of_mem_diff_closure (he : e ∈ M.E \ M.closure X) : e ∉ X :=
  fun heX ↦ he.2 <| M.mem_closure_of_mem' heX he.1

@[aesop unsafe 10% (rule_sets := [Matroid])]
theorem mem_ground_of_mem_closure (he : e ∈ M.closure X) : e ∈ M.E :=
...
theorem closure_biUnion_closure_eq_closure_sUnion (M : Matroid α) (Xs : Set (Set α)) :
...
theorem closure_closure_union_closure_eq_closure_union (M : Matroid α) (X Y : Set α) :
...
@[simp] theorem closure_union_closure_right_eq (M : Matroid α) (X Y : Set α) :
...
theorem Indep.closure_eq_setOf_basis_insert (hI : M.Indep I) :
...
theorem Indep.insert_basis_iff_mem_closure (hI : M.Indep I) :
    M.Basis I (insert e I) ↔ e ∈ M.closure I := by
  rw [hI.closure_eq_setOf_basis_insert, mem_setOf]

theorem Indep.basis_closure (hI : M.Indep I) : M.Basis I (M.closure I) := by
  rw [hI.closure_eq_setOf_basis_insert]; exact hI.basis_setOf_insert_basis

theorem Basis.closure_eq_closure (h : M.Basis I X) : M.closure I = M.closure X := by
...
theorem Basis.closure_eq_right (h : M.Basis I (M.closure X)) : M.closure I = M.closure X :=
  M.closure_closure X ▸ h.closure_eq_closure

theorem Basis'.closure_eq_closure (h : M.Basis' I X) : M.closure I = M.closure X := by
  rw [← closure_inter_ground _ X, h.basis_inter_ground.closure_eq_closure]

theorem Basis.subset_closure (h : M.Basis I X) : X ⊆ M.closure I := by
  rw [← closure_subset_closure_iff_subset_closure, h.closure_eq_closure]

theorem Basis'.basis_closure_right (h : M.Basis' I X) : M.Basis I (M.closure X) := by
  rw [← h.closure_eq_closure]; exact h.indep.basis_closure

theorem Basis.basis_closure_right (h : M.Basis I X) : M.Basis I (M.closure X) :=
  h.basis'.basis_closure_right

theorem Indep.mem_closure_iff (hI : M.Indep I) :
...
theorem Indep.insert_dep_iff (hI : M.Indep I) : M.Dep (insert e I) ↔ e ∈ M.closure I \ I := by
...
theorem Indep.mem_closure_iff_of_not_mem (hI : M.Indep I) (heI : e ∉ I) :
    e ∈ M.closure I ↔ M.Dep (insert e I) := by
  rw [hI.insert_dep_iff, mem_diff, and_iff_left heI]

theorem Indep.not_mem_closure_iff (hI : M.Indep I) (he : e ∈ M.E := by aesop_mat) :
...
theorem Indep.not_mem_closure_iff_of_not_mem (hI : M.Indep I) (heI : e ∉ I)
...
theorem insert_indep_iff : M.Indep (insert e I) ↔ M.Indep I ∧ (e ∉ I → e ∈ M.E \ M.closure I) := by
...
theorem Indep.insert_diff_indep_iff (hI : M.Indep (I \ {e})) (heI : e ∈ I) :
...
theorem Indep.basis_of_subset_of_subset_closure (hI : M.Indep I) (hIX : I ⊆ X)
    (hXI : X ⊆ M.closure I) : M.Basis I X :=
  hI.basis_closure.basis_subset hIX hXI

theorem basis_iff_indep_subset_closure : M.Basis I X ↔ M.Indep I ∧ I ⊆ X ∧ X ⊆ M.closure I :=
  ⟨fun h ↦ ⟨h.indep, h.subset, h.subset_closure⟩,
    fun h ↦ h.1.basis_of_subset_of_subset_closure h.2.1 h.2.2⟩

theorem Indep.base_of_ground_subset_closure (hI : M.Indep I) (h : M.E ⊆ M.closure I) : M.Base I := by
  rw [← basis_ground_iff]; exact hI.basis_of_subset_of_subset_closure hI.subset_ground h

theorem Base.closure_eq (hB : M.Base B) : M.closure B = M.E := by
  rw [← basis_ground_iff] at hB; rw [hB.closure_eq_closure, closure_ground]

theorem Base.closure_of_superset (hB : M.Base B) (hBX : B ⊆ X) : M.closure X = M.E :=
  (M.closure_subset_ground _).antisymm (hB.closure_eq ▸ M.closure_subset_closure hBX)

theorem base_iff_indep_closure_eq : M.Base B ↔ M.Indep B ∧ M.closure B = M.E := by
...
theorem Indep.base_iff_ground_subset_closure (hI : M.Indep I) : M.Base I ↔ M.E ⊆ M.closure I :=
  ⟨fun h ↦ h.closure_eq.symm.subset, hI.base_of_ground_subset_closure⟩

theorem Indep.closure_inter_eq_self_of_subset (hI : M.Indep I) (hJI : J ⊆ I) :
    M.closure J ∩ I = J := by
...
theorem Indep.closure_sInter_eq_biInter_closure_of_forall_subset {Js : Set (Set α)} (hI : M.Indep I)
...
theorem closure_iInter_eq_iInter_closure_of_iUnion_indep [hι : Nonempty ι] (Is : ι → Set α)
...
theorem closure_sInter_eq_biInter_closure_of_sUnion_indep (Is : Set (Set α)) (hIs : Is.Nonempty)
...
theorem closure_biInter_eq_biInter_closure_of_biUnion_indep {ι : Type*} {A : Set ι} (hA : A.Nonempty)
...
theorem Indep.closure_iInter_eq_biInter_closure_of_forall_subset [Nonempty ι] {Js : ι → Set α}
...
theorem Indep.closure_inter_eq_inter_closure (h : M.Indep (I ∪ J)) :
...
theorem basis_iff_basis_closure_of_subset (hIX : I ⊆ X) (hX : X ⊆ M.E := by aesop_mat) :
    M.Basis I X ↔ M.Basis I (M.closure X) :=
  ⟨fun h ↦ h.basis_closure_right, fun h ↦ h.basis_subset hIX (M.subset_closure X hX)⟩

theorem basis_iff_basis_closure_of_subset' (hIX : I ⊆ X) :
...
theorem basis'_iff_basis_closure : M.Basis' I X ↔ M.Basis I (M.closure X) ∧ I ⊆ X := by
...
theorem exists_basis_inter_ground_basis_closure (M : Matroid α) (X : Set α) :
...
theorem Basis.basis_of_closure_eq_closure (hI : M.Basis I X) (hY : I ⊆ Y)
...
theorem basis_union_iff_indep_closure : M.Basis I (I ∪ X) ↔ M.Indep I ∧ X ⊆ M.closure I :=
...
theorem basis_iff_indep_closure : M.Basis I X ↔ M.Indep I ∧ X ⊆ M.closure I ∧ I ⊆ X :=
...
theorem Basis.eq_of_closure_subset (hI : M.Basis I X) (hJI : J ⊆ I) (hJ : X ⊆ M.closure J) :
    J = I := by
  rw [← hI.indep.closure_inter_eq_self_of_subset hJI, inter_eq_self_of_subset_right]
  exact hI.subset.trans hJ

@[simp] theorem empty_basis_iff : M.Basis ∅ X ↔ X ⊆ M.closure ∅ := by
  rw [basis_iff_indep_closure, and_iff_right M.empty_indep, and_iff_left (empty_subset _)]

theorem indep_iff_forall_not_mem_closure_diff (hI : I ⊆ M.E := by aesop_mat) :
...
theorem indep_iff_forall_not_mem_closure_diff' :
...
theorem Indep.not_mem_closure_diff_of_mem (hI : M.Indep I) (he : e ∈ I) : e ∉ M.closure (I \ {e}) :=
  (indep_iff_forall_not_mem_closure_diff'.1 hI).2 e he

theorem indep_iff_forall_closure_diff_ne :
...
theorem Indep.closure_ssubset_closure (hI : M.Indep I) (hJI : J ⊂ I) : M.closure J ⊂ M.closure I := by
...
theorem indep_iff_forall_closure_ssubset_of_ssubset (hI : I ⊆ M.E := by aesop_mat) :
...
theorem Indep.closure_diff_ssubset (hI : M.Indep I) (hX : (I ∩ X).Nonempty) :
...
theorem Indep.closure_diff_singleton_ssubset (hI : M.Indep I) (he : e ∈ I) :
...
theorem mem_closure_insert (he : e ∉ M.closure X) (hef : e ∈ M.closure (insert f X)) :
...
theorem closure_exchange (he : e ∈ M.closure (insert f X) \ M.closure X) :
...
theorem closure_diff_singleton_eq_closure (h : e ∈ M.closure (X \ {e})) :
    M.closure (X \ {e}) = M.closure X :=
  closure_diff_eq_self (by simpa)

theorem subset_closure_diff_iff_closure_eq (h : Y ⊆ X) (hY : Y ⊆ M.E := by aesop_mat) :
    Y ⊆ M.closure (X \ Y) ↔ M.closure (X \ Y) = M.closure X :=
  ⟨closure_diff_eq_self, fun h' ↦ (M.subset_closure_of_subset' h).trans h'.symm.subset⟩

theorem mem_closure_diff_singleton_iff_closure (he : e ∈ X) (heE : e ∈ M.E := by aesop_mat) :
...
theorem Spanning.closure_eq_of_superset (hS : M.Spanning S) (hST : S ⊆ T) : M.closure T = M.E := by
  rw [← closure_inter_ground, ← spanning_iff_closure_eq]
  exact hS.superset (subset_inter hST hS.subset_ground)

theorem Spanning.union_left (hS : M.Spanning S) (hX : X ⊆ M.E := by aesop_mat) : M.Spanning (S ∪ X) :=
...
theorem Base.spanning_of_superset (hB : M.Base B) (hBX : B ⊆ X) (hX : X ⊆ M.E := by aesop_mat) :
    M.Spanning X :=
...
theorem spanning_iff_exists_base_subset' : M.Spanning S ↔ (∃ B, M.Base B ∧ B ⊆ S) ∧ S ⊆ M.E := by
...
theorem spanning_iff_exists_base_subset (hS : S ⊆ M.E := by aesop_mat) :
...
theorem coindep_iff_closure_compl_eq_ground (hK : X ⊆ M.E := by aesop_mat) :
    M.Coindep X ↔ M.closure (M.E \ X) = M.E := by
  rw [coindep_iff_compl_spanning, spanning_iff_closure_eq]

theorem Coindep.closure_compl (hX : M.Coindep X) : M.closure (M.E \ X) = M.E :=
...
theorem ext_spanning {M M' : Matroid α} (h : M.E = M'.E)
