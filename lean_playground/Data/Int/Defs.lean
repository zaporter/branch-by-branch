...
This file contains some basic theorems about integers.
...
* `Data.Int.Init` (or maybe `Data.Int.Batteries`?) for theorems that could go to Batteries
* `Data.Int.Basic` for the theorems that require mathlib definitions
-/
...
protected theorem lt_or_le (a b : ℤ) : a < b ∨ b ≤ a := by rw [← Int.not_lt]; exact em _
protected theorem le_or_lt (a b : ℤ) : a ≤ b ∨ b < a := (b.lt_or_le a).symm
...
protected theorem le_antisymm_iff : a = b ↔ a ≤ b ∧ b ≤ a :=
  ⟨fun h ↦ ⟨Int.le_of_eq h, Int.ge_of_eq h⟩, fun h ↦ Int.le_antisymm h.1 h.2⟩
protected theorem le_iff_eq_or_lt : a ≤ b ↔ a = b ∨ a < b := by
...
protected theorem one_ne_zero : (1 : ℤ) ≠ 0 := by decide

protected theorem one_nonneg : 0 ≤ (1 : ℤ) := Int.le_of_lt Int.zero_lt_one

theorem zero_le_ofNat (n : ℕ) : 0 ≤ ofNat n := @le.intro _ _ n (by rw [Int.zero_add]; rfl)

protected theorem neg_eq_neg {a b : ℤ} (h : -a = -b) : a = b := Int.neg_inj.1 h

-- We want to use these theorems earlier than the theorems simp can prove them with
@[simp, nolint simpNF]
protected theorem neg_pos : 0 < -a ↔ a < 0 := ⟨Int.neg_of_neg_pos, Int.neg_pos_of_neg⟩

@[simp, nolint simpNF]
protected theorem neg_nonneg : 0 ≤ -a ↔ a ≤ 0 := ⟨Int.nonpos_of_neg_nonneg, Int.neg_nonneg_of_nonpos⟩

@[simp, nolint simpNF]
protected theorem neg_neg_iff_pos : -a < 0 ↔ 0 < a := ⟨Int.pos_of_neg_neg, Int.neg_neg_of_pos⟩

@[simp, nolint simpNF]
protected theorem neg_nonpos_iff_nonneg : -a ≤ 0 ↔ 0 ≤ a :=
...
@[simp] theorem ofNat_injective : Function.Injective ofNat := @Int.ofNat.inj

@[simp] theorem ofNat_eq_natCast (n : ℕ) : Int.ofNat n = n := rfl

@[norm_cast] theorem natCast_inj {m n : ℕ} : (m : ℤ) = (n : ℤ) ↔ m = n := ofNat_inj

@[simp, norm_cast] theorem natAbs_cast (n : ℕ) : natAbs ↑n = n := rfl

@[norm_cast]
protected theorem natCast_sub {n m : ℕ} : n ≤ m → (↑(m - n) : ℤ) = ↑m - ↑n := ofNat_sub

-- We want to use this theorem earlier than the theorems simp can prove it with
@[simp, nolint simpNF] theorem natCast_eq_zero {n : ℕ} : (n : ℤ) = 0 ↔ n = 0 := by omega

theorem natCast_ne_zero {n : ℕ} : (n : ℤ) ≠ 0 ↔ n ≠ 0 := by omega

theorem natCast_ne_zero_iff_pos {n : ℕ} : (n : ℤ) ≠ 0 ↔ 0 < n := by omega

-- We want to use this theorem earlier than the theorems simp can prove it with
@[simp, nolint simpNF] theorem natCast_pos {n : ℕ} : (0 : ℤ) < n ↔ 0 < n := by omega

theorem natCast_succ_pos (n : ℕ) : 0 < (n.succ : ℤ) := natCast_pos.2 n.succ_pos

-- We want to use this theorem earlier than the theorems simp can prove it with
@[simp, nolint simpNF] theorem natCast_nonpos_iff {n : ℕ} : (n : ℤ) ≤ 0 ↔ n = 0 := by omega

theorem natCast_nonneg (n : ℕ) : 0 ≤ (n : ℤ) := ofNat_le.2 (Nat.zero_le _)

@[simp] theorem sign_natCast_add_one (n : ℕ) : sign (n + 1) = 1 := rfl

@[simp, norm_cast] theorem cast_id {n : ℤ} : Int.cast n = n := rfl

protected theorem two_mul : ∀ n : ℤ, 2 * n = n + n
...
protected theorem mul_le_mul_iff_of_pos_right (ha : 0 < a) : b * a ≤ c * a ↔ b ≤ c :=
  ⟨(le_of_mul_le_mul_right · ha), (Int.mul_le_mul_of_nonneg_right · (Int.le_of_lt ha))⟩

protected theorem mul_nonneg_iff_of_pos_right (hb : 0 < b) : 0 ≤ a * b ↔ 0 ≤ a := by
...
theorem natCast_succ (n : ℕ) : (Nat.succ n : ℤ) = Int.succ n := rfl

theorem pred_succ (a : ℤ) : pred (succ a) = a := Int.add_sub_cancel _ _

theorem succ_pred (a : ℤ) : succ (pred a) = a := Int.sub_add_cancel _ _

theorem neg_succ (a : ℤ) : -succ a = pred (-a) := Int.neg_add
...
theorem succ_neg_natCast_succ (n : ℕ) : succ (-Nat.succ n) = -n := succ_neg_succ n

@[norm_cast] theorem natCast_pred_of_pos {n : ℕ} (h : 0 < n) : ((n - 1 : ℕ) : ℤ) = (n : ℤ) - 1 := by
  cases n; cases h; simp [ofNat_succ]

theorem lt_succ_self (a : ℤ) : a < succ a := by unfold succ; omega

theorem pred_self_lt (a : ℤ) : pred a < a := by unfold pred; omega

theorem le_add_one_iff : m ≤ n + 1 ↔ m ≤ n ∨ m = n + 1 := by omega

theorem sub_one_lt_iff : m - 1 < n ↔ m ≤ n := by omega

theorem le_sub_one_iff : m ≤ n - 1 ↔ m < n := by omega

section
open Lean.Omega.Int

/-!
The following few theorems are proved in the core implementation of the `omega` tactic. We expose
them here with nice user-facing names.
-/

protected theorem add_le_iff_le_sub : a + b ≤ c ↔ a ≤ c - b := add_le_iff_le_sub ..
protected theorem le_add_iff_sub_le : a ≤ b + c ↔ a - c ≤ b := le_add_iff_sub_le ..
protected theorem add_le_zero_iff_le_neg : a + b ≤ 0 ↔ a ≤ - b := add_le_zero_iff_le_neg ..
protected theorem add_le_zero_iff_le_neg' : a + b ≤ 0 ↔ b ≤ -a := add_le_zero_iff_le_neg' ..
protected theorem add_nonnneg_iff_neg_le : 0 ≤ a + b ↔ -b ≤ a := add_nonnneg_iff_neg_le ..
protected theorem add_nonnneg_iff_neg_le' : 0 ≤ a + b ↔ -a ≤ b := add_nonnneg_iff_neg_le' ..
...
theorem inductionOn'_self : b.inductionOn' b H0 Hs Hp = H0 :=
...
variable {lt ge}
theorem strongRec_of_lt (hn : n < m) : m.strongRec lt ge n = lt n hn := dif_pos _

theorem strongRec_of_ge :
...
@[simp] theorem natAbs_ofNat' (n : ℕ) : natAbs (ofNat n) = n := rfl
...
theorem pow_right_injective (h : 1 < a.natAbs) : ((a ^ ·) : ℕ → ℤ).Injective := by
...
@[simp, norm_cast] theorem natCast_div (m n : ℕ) : ((m / n : ℕ) : ℤ) = m / n := rfl

theorem natCast_ediv (m n : ℕ) : ((m / n : ℕ) : ℤ) = ediv m n := rfl
...
protected theorem mul_dvd_mul_left (a : ℤ) (h : b ∣ c) : a * b ∣ a * c := Int.mul_dvd_mul a.dvd_refl h

protected theorem mul_dvd_mul_right (a : ℤ) (h : b ∣ c) : b * a ∣ c * a :=
  Int.mul_dvd_mul h a.dvd_refl

theorem dvd_mul_of_div_dvd (h : b ∣ a) (hdiv : a / b ∣ c) : a ∣ b * c := by
  obtain ⟨e, rfl⟩ := hdiv
  rw [← Int.mul_assoc, Int.mul_comm _ (a / b), Int.ediv_mul_cancel h]
  exact Int.dvd_mul_right a e

@[simp] theorem div_dvd_iff_dvd_mul (h : b ∣ a) (hb : b ≠ 0) : a / b ∣ c ↔ a ∣ b * c :=
...
theorem dvd_div_of_mul_dvd (h : a * b ∣ c) : b ∣ c / a := by
  obtain rfl | ha := eq_or_ne a 0
  · simp
  · obtain ⟨d, rfl⟩ := h
    simp [Int.mul_assoc, ha]

@[simp] theorem dvd_div_iff_mul_dvd (hbc : c ∣ b) : a ∣ b / c ↔ c * a ∣ b :=
...
/-- If `n > 0` then `m` is not divisible by `n` iff it is between `n * k` and `n * (k + 1)`
  for some `k`. -/
theorem exists_lt_and_lt_iff_not_dvd (m : ℤ) (hn : 0 < n) :
...
@[norm_cast] theorem natCast_dvd_natCast {m n : ℕ} : (↑m : ℤ) ∣ ↑n ↔ m ∣ n where
  mp := by
    rintro ⟨a, h⟩
...
theorem dvd_of_mul_dvd_mul_left (ha : a ≠ 0) (h : a * m ∣ a * n) : m ∣ n := by
  obtain ⟨b, hb⟩ := h
  rw [Int.mul_assoc, Int.mul_eq_mul_left_iff ha] at hb
  exact ⟨_, hb⟩

theorem dvd_of_mul_dvd_mul_right (ha : a ≠ 0) (h : m * a ∣ n * a) : m ∣ n :=
  dvd_of_mul_dvd_mul_left ha (by simpa [Int.mul_comm] using h)

theorem eq_mul_div_of_mul_eq_mul_of_dvd_left (hb : b ≠ 0) (hbc : b ∣ c) (h : b * a = c * d) :
    a = c / b * d := by
  obtain ⟨k, rfl⟩ := hbc
...
theorem eq_zero_of_dvd_of_natAbs_lt_natAbs (hmn : m ∣ n) (hnm : natAbs n < natAbs m) : n = 0 := by
  rw [← natAbs_dvd, ← dvd_natAbs, natCast_dvd_natCast] at hmn
  rw [← natAbs_eq_zero]
  exact Nat.eq_zero_of_dvd_of_lt hmn hnm

theorem eq_zero_of_dvd_of_nonneg_of_lt (hm : 0 ≤ m) (hmn : m < n) (hnm : n ∣ m) : m = 0 :=
...
theorem natAbs_le_of_dvd_ne_zero (hmn : m ∣ n) (hn : n ≠ 0) : natAbs m ≤ natAbs n :=
...
theorem ediv_dvd_of_dvd (hmn : m ∣ n) : n / m ∣ n := by
  obtain rfl | hm := eq_or_ne m 0
  · simpa using hmn
  · obtain ⟨a, ha⟩ := hmn
    simp [ha, Int.mul_ediv_cancel_left _ hm, Int.dvd_mul_left]

theorem le_iff_pos_of_dvd (ha : 0 < a) (hab : a ∣ b) : a ≤ b ↔ 0 < b :=
  ⟨Int.lt_of_lt_of_le ha, (Int.le_of_dvd · hab)⟩

theorem le_add_iff_lt_of_dvd_sub (ha : 0 < a) (hab : a ∣ c - b) : a + b ≤ c ↔ b < c := by
  rw [Int.add_le_iff_le_sub, ← Int.sub_pos, le_iff_pos_of_dvd ha hab]

/-! ### sign -/

theorem sign_natCast_of_ne_zero {n : ℕ} (hn : n ≠ 0) : Int.sign n = 1 := sign_ofNat_of_nonzero hn
...
@[simp] theorem toNat_natCast (n : ℕ) : toNat ↑n = n := rfl

@[simp] theorem toNat_natCast_add_one {n : ℕ} : ((n : ℤ) + 1).toNat = n + 1 := rfl

@[simp] theorem toNat_le {n : ℕ} : toNat m ≤ n ↔ m ≤ n := by
  rw [ofNat_le.symm, toNat_eq_max, Int.max_le]; exact and_iff_left (ofNat_zero_le _)

@[simp]
theorem lt_toNat {m : ℕ} : m < toNat n ↔ (m : ℤ) < n := by rw [← Int.not_le, ← Nat.not_le, toNat_le]

theorem toNat_le_toNat {a b : ℤ} (h : a ≤ b) : toNat a ≤ toNat b := by
  rw [toNat_le]; exact Int.le_trans h (self_le_toNat b)

theorem toNat_lt_toNat {a b : ℤ} (hb : 0 < b) : toNat a < toNat b ↔ a < b where
...
@[simp] theorem toNat_pred_coe_of_pos {i : ℤ} (h : 0 < i) : ((i.toNat - 1 : ℕ) : ℤ) = i - 1 := by
  simp only [lt_toNat, Nat.cast_ofNat_Int, h, natCast_pred_of_pos, Int.le_of_lt h, toNat_of_nonneg]

@[simp] theorem toNat_eq_zero : ∀ {n : ℤ}, n.toNat = 0 ↔ n ≤ 0
  | (n : ℕ) => by simp
...
theorem toNat_lt' {n : ℕ} (hn : n ≠ 0) : m.toNat < n ↔ m < n := by
...
theorem natMod_lt {n : ℕ} (hn : n ≠ 0) : m.natMod n < n :=
