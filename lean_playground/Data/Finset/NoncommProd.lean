...
elements is commutative and using the `Finset.prod` versions of theorems to prove the `noncommProd`
version.
-/
...
@[to_additive]
theorem noncommProd_induction (s : Multiset α) (comm)
...
@[to_additive]
theorem noncommProd_theorem (s : Finset α) (f : α → β)
...
def noncommProd (s : Finset α) (f : α → β)
  ...
  (s.1.map f).noncommProd <| noncommProd_theorem s f comm

@[to_additive]
theorem noncommProd_induction (s : Finset α) (f : α → β) (comm)
