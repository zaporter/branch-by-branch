...
theorem finite_length_eq : {l : List α | l.length = n}.Finite := List.Vector.finite

theorem finite_length_lt : {l : List α | l.length < n}.Finite := by
  convert (Finset.range n).finite_toSet.biUnion fun i _ ↦ finite_length_eq α i; ext; simp

theorem finite_length_le : {l : List α | l.length ≤ n}.Finite := by
