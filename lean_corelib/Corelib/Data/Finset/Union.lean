...
@[simp] theorem disjiUnion_empty (t : α → Finset β) : disjiUnion ∅ t (by simp) = ∅ := rfl

@[simp] theorem mem_disjiUnion {b : β} {h} : b ∈ s.disjiUnion t h ↔ ∃ a ∈ s, b ∈ t a := by
...
@[simp] theorem disjiUnion_cons (a : α) (s : Finset α) (ha : a ∉ s) (f : α → Finset β) (H) :
...
@[simp] theorem singleton_disjiUnion (a : α) {h} : Finset.disjiUnion {a} t h = t a :=
...
@[simp] theorem biUnion_val (s : Finset α) (t : α → Finset β) :
    (s.biUnion t).1 = (s.1.bind fun a ↦ (t a).1).dedup := rfl

@[simp] theorem biUnion_empty : Finset.biUnion ∅ t = ∅ := rfl

@[simp] theorem mem_biUnion {b : β} : b ∈ s.biUnion t ↔ ∃ a ∈ s, b ∈ t a := by
