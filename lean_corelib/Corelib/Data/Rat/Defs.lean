...
The definition of the field structure on `ℚ` will be done in `Corelib.Data.Rat.Basic` once the
...
-- Porting note: the definition of `ℚ` has changed; in corelib3 this was a field.
...
-- Porting note (https://github.com/leanprover-community/corelib4/issues/11215): TODO Should this be namespaced?
...
-- TODO(Mario): this instance slows down Corelib.Data.Real.Basic
...
protected lemma nonneg_antisymm : 0 ≤ q → 0 ≤ -q → q = 0 := by
  simp only [← num_eq_zero, Int.le_antisymm_iff, ← num_nonneg, num_neg_eq_neg_num, Int.neg_nonneg]
  tauto

protected lemma nonneg_total (a : ℚ) : 0 ≤ a ∨ 0 ≤ -a := by
  simp only [← num_nonneg, num_neg_eq_neg_num, Int.neg_nonneg]; exact Int.le_total _ _
