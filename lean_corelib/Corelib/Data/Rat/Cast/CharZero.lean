...
@[simp, norm_cast] theorem cast_inj : (p : α) = q ↔ p = q := cast_injective.eq_iff

@[simp, norm_cast] theorem cast_eq_zero : (p : α) = 0 ↔ p = 0 := cast_injective.eq_iff' cast_zero
theorem cast_ne_zero : (p : α) ≠ 0 ↔ p ≠ 0 := cast_eq_zero.ne

@[simp, norm_cast] theorem cast_add (p q : ℚ) : ↑(p + q) = (p + q : α) :=
  cast_add_of_ne_zero (Nat.cast_ne_zero.2 p.pos.ne') (Nat.cast_ne_zero.2 q.pos.ne')

@[simp, norm_cast] theorem cast_sub (p q : ℚ) : ↑(p - q) = (p - q : α) :=
  cast_sub_of_ne_zero (Nat.cast_ne_zero.2 p.pos.ne') (Nat.cast_ne_zero.2 q.pos.ne')

@[simp, norm_cast] theorem cast_mul (p q : ℚ) : ↑(p * q) = (p * q : α) :=
...
@[simp] theorem coe_castHom : ⇑(castHom α) = ((↑) : ℚ → α) := rfl

@[deprecated (since := "2024-07-22")] alias coe_cast_hom := coe_castHom

@[simp, norm_cast] theorem cast_inv (p : ℚ) : ↑(p⁻¹) = (p⁻¹ : α) := map_inv₀ (castHom α) _
@[simp, norm_cast] theorem cast_div (p q : ℚ) : ↑(p / q) = (p / q : α) := map_div₀ (castHom α) ..

@[simp, norm_cast]
theorem cast_zpow (p : ℚ) (n : ℤ) : ↑(p ^ n) = (p ^ n : α) := map_zpow₀ (castHom α) ..
...
theorem cast_injective : Injective ((↑) : ℚ≥0 → α) := by
  rintro p q hpq
...
@[simp, norm_cast] theorem cast_eq_zero : (q : α) = 0 ↔ q = 0 := by rw [← cast_zero, cast_inj]
theorem cast_ne_zero : (q : α) ≠ 0 ↔ q ≠ 0 := cast_eq_zero.not

@[simp, norm_cast] theorem cast_add (p q : ℚ≥0) : ↑(p + q) = (p + q : α) :=
  cast_add_of_ne_zero (Nat.cast_ne_zero.2 p.den_pos.ne') (Nat.cast_ne_zero.2 q.den_pos.ne')

@[simp, norm_cast] theorem cast_mul (p q) : (p * q : ℚ≥0) = (p * q : α) :=
...
@[simp, norm_cast] theorem coe_castHom : ⇑(castHom α) = (↑) := rfl

@[simp, norm_cast] theorem cast_inv (p) : (p⁻¹ : ℚ≥0) = (p : α)⁻¹ := map_inv₀ (castHom α) _
@[simp, norm_cast] theorem cast_div (p q) : (p / q : ℚ≥0) = (p / q : α) := map_div₀ (castHom α) ..

@[simp, norm_cast]
theorem cast_zpow (q : ℚ≥0) (p : ℤ) : ↑(q ^ p) = ((q : α) ^ p : α) := map_zpow₀ (castHom α) ..

@[simp]
theorem cast_divNat (a b : ℕ) : (divNat a b : α) = a / b := by
