...
* `Data.Nat.Init` (or maybe `Data.Nat.Batteries`?) for theorems that could go to Batteries
* `Data.Nat.Basic` for the theorems that require mathlib definitions
-/
...
theorem one_lt_succ_succ (n : ℕ) : 1 < n.succ.succ := succ_lt_succ <| succ_pos n
...
protected theorem min_left_comm (a b c : ℕ) : min a (min b c) = min b (min a c) := by
  rw [← Nat.min_assoc, ← Nat.min_assoc, b.min_comm]

protected theorem max_left_comm (a b c : ℕ) : max a (max b c) = max b (max a c) := by
  rw [← Nat.max_assoc, ← Nat.max_assoc, b.max_comm]

protected theorem min_right_comm (a b c : ℕ) : min (min a b) c = min (min a c) b := by
  rw [Nat.min_assoc, Nat.min_assoc, b.min_comm]

protected theorem max_right_comm (a b c : ℕ) : max (max a b) c = max (max a c) b := by
  rw [Nat.max_assoc, Nat.max_assoc, b.max_comm]
...
-- We want to use these two theorems earlier than the theorems simp can prove them with
@[simp, nolint simpNF] protected theorem add_eq_left : a + b = a ↔ b = 0 := by omega
@[simp, nolint simpNF] protected theorem add_eq_right : a + b = b ↔ a = 0 := by omega
...
@[simp, nolint simpNF] protected theorem add_eq_zero : m + n = 0 ↔ m = 0 ∧ n = 0 := by omega
...
protected theorem sub_eq_of_eq_add' (h : a = b + c) : a - b = c := by omega
protected theorem eq_sub_of_add_eq (h : c + b = a) : c = a - b := by omega
protected theorem eq_sub_of_add_eq' (h : b + c = a) : c = a - b := by omega
...
theorem mul_self_inj : m * m = n * n ↔ m = n := by
...
protected theorem add_le_mul {a : ℕ} (ha : 2 ≤ a) : ∀ {b : ℕ} (_ : 2 ≤ b), a + b ≤ a * b
  | 2, _ => by omega
...
theorem div_lt_iff_lt_mul' (hb : 0 < b) : a / b < c ↔ a < c * b := div_lt_iff_lt_mul hb
...
protected theorem div_pos (hba : b ≤ a) (hb : 0 < b) : 0 < a / b := Nat.div_pos_iff.2 ⟨hb, hba⟩
...
protected theorem mul_div_mul_comm (hba : b ∣ a) (hdc : d ∣ c) : a * c / (b * d) = a / b * (c / d) :=
...
protected theorem div_le_self' (m n : ℕ) : m / n ≤ m := by
...
protected theorem mul_le_of_le_div (k x y : ℕ) (h : x ≤ y / k) : x * k ≤ y := by
  if hk : k = 0 then
    rw [hk, Nat.mul_zero]; exact zero_le _
  else
    rwa [← le_div_iff_mul_le (Nat.pos_iff_ne_zero.2 hk)]

protected theorem div_mul_div_le (a b c d : ℕ) :
    (a / b) * (c / d) ≤ (a * c) / (b * d) := by
...
protected theorem pow_lt_pow_left (h : a < b) : ∀ {n : ℕ}, n ≠ 0 → a ^ n < b ^ n
  | 1, _ => by simpa
...
protected theorem pow_le_pow_iff_left {n : ℕ} (hn : n ≠ 0) : a ^ n ≤ b ^ n ↔ a ≤ b where
...
-- We want to use this theorem earlier than the theorem simp can prove it with
@[simp, nolint simpNF] protected theorem pow_eq_zero {a : ℕ} : ∀ {n : ℕ}, a ^ n = 0 ↔ a = 0 ∧ n ≠ 0
  | 0 => by simp
  | n + 1 => by rw [Nat.pow_succ, mul_eq_zero, Nat.pow_eq_zero]; omega

/-- For `a > 1`, `a ^ b = a` iff `b = 1`. -/
theorem pow_eq_self_iff {a b : ℕ} (ha : 1 < a) : a ^ b = a ↔ b = 1 :=
...
theorem rec_add_one {C : ℕ → Sort*} (h0 : C 0) (h : ∀ n, C n → C (n + 1)) (n : ℕ) :
    Nat.rec h0 h (n + 1) = h n (Nat.rec h0 h n) := rfl

@[simp] theorem rec_one {C : ℕ → Sort*} (h0 : C 0) (h : ∀ n, C n → C (n + 1)) :
...
@[simp]
theorem leRec_self {n} {motive : (m : ℕ) → n ≤ m → Sort*}
    (refl : motive n le_rfl)
    (le_succ_of_le : ∀ ⦃k⦄ (h : n ≤ k), motive k h → motive (k + 1) (le_succ_of_le h)) :
...
@[simp]
theorem leRec_succ {n} {motive : (m : ℕ) → n ≤ m → Sort*}
    (refl : motive n le_rfl)
    (le_succ_of_le : ∀ ⦃k⦄ (h : n ≤ k), motive k h → motive (k + 1) (le_succ_of_le h))
    (h1 : n ≤ m) {h2 : n ≤ m + 1} :
...
theorem leRec_succ' {n} {motive : (m : ℕ) → n ≤ m → Sort*} (refl le_succ_of_le) :
    (leRec (motive := motive) refl le_succ_of_le (le_succ _)) = le_succ_of_le _ refl := by
  rw [leRec_succ, leRec_self]

theorem leRec_trans {n m k} {motive : (m : ℕ) → n ≤ m → Sort*} (refl le_succ_of_le)
    (hnm : n ≤ m) (hmk : m ≤ k) :
...
theorem leRec_succ_left {motive : (m : ℕ) → n ≤ m → Sort*}
...
theorem leRecOn_succ_left {C : ℕ → Sort*} {n m}
...
@[simp]
theorem decreasingInduction_self {n} {motive : (m : ℕ) → m ≤ n → Sort*} (of_succ self) :
    (decreasingInduction (motive := motive) of_succ self le_rfl) = self := by
  dsimp only [decreasingInduction]
  rw [leRec_self]

theorem decreasingInduction_succ {n} {motive : (m : ℕ) → m ≤ n + 1 → Sort*} (of_succ self)
    (mn : m ≤ n) (msn : m ≤ n + 1) :
...
theorem decreasingInduction_trans {motive : (m : ℕ) → m ≤ k → Sort*} (hmn : m ≤ n) (hnk : n ≤ k)
    (of_succ self) :
...
theorem decreasingInduction_succ_left  {motive : (m : ℕ) → m ≤ n → Sort*} (of_succ self)
    (smn : m + 1 ≤ n) (mn : m ≤ n) :
...
protected theorem lt_div_iff_mul_lt' (hdn : d ∣ n) (a : ℕ) : a < n / d ↔ d * a < n := by
  obtain rfl | hd := d.eq_zero_or_pos
...
theorem mul_div_lt_iff_not_dvd : d * (n / d) < n ↔ ¬ d ∣ n := by
...
theorem one_mod_eq_one : ∀ {n : ℕ}, 1 % n = 1 ↔ n ≠ 1
  | 0 | 1 | n + 2 => by simp

@[deprecated "No deprecation message was provided." (since := "2024-08-28")]
theorem one_mod_of_ne_one  : ∀ {n : ℕ}, n ≠ 1 → 1 % n = 1 := one_mod_eq_one.mpr
...
@[simp] protected theorem dvd_add_self_left : m ∣ m + n ↔ m ∣ n := Nat.dvd_add_right (Nat.dvd_refl m)

/-- A natural number `m` divides the sum `n + m` if and only if `m` divides `n`.-/
@[simp] protected theorem dvd_add_self_right : m ∣ n + m ↔ m ∣ n := Nat.dvd_add_left (Nat.dvd_refl m)

-- TODO: update `Nat.dvd_sub` in core
theorem dvd_sub' (h₁ : k ∣ m) (h₂ : k ∣ n) : k ∣ m - n := by
...
theorem succ_div_of_dvd (hba : b ∣ a + 1) : (a + 1) / b = a / b + 1 := by rw [succ_div, if_pos hba]

theorem succ_div_of_not_dvd (hba : ¬b ∣ a + 1) : (a + 1) / b = a / b := by
...
theorem dvd_div_of_mul_dvd (h : a * b ∣ c) : b ∣ c / a :=
...
@[simp] theorem div_div_div_eq_div (dvd : b ∣ a) (dvd2 : a ∣ c) : c / (a / b) / b = c / a :=
  match a, b, c with
  | 0, _, _ => by simp
